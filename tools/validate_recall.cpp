...
std::vector<float> parse_components(const std::vector<std::string>& fields,
...
struct VectorDataset {
  ...
  std::vector<fast_vector::VectorId> ids;
  std::vector<std::vector<float>> vectors;
};

VectorDataset load_vectors(const std::string& path) {
  ...
  VectorDataset dataset{dimension, {}, {}};
  ...
  while (std::getline(input, line)) {
    ...
    dataset.ids.push_back(parse_id(fields.front()));
    dataset.vectors.push_back(parse_components(fields, dimension));
  }
  return dataset;
}
...
std::uint64_t parse_seed(const std::string_view text) {
  std::uint64_t seed = 0;
...
fast_vector::HnswNeighborSelection parse_neighbor_selection(const std::string_view text) {
  if (text == "simple") {
    return fast_vector::HnswNeighborSelection::Simple;
  }
  if (text == "heuristic") {
    return fast_vector::HnswNeighborSelection::Heuristic;
  }
...
int main(const int argc, char* argv[]) {
  if (argc != 5 && argc != 6 && argc != 12) {
    std::cerr << "Usage: fast_vector_validate <vectors.csv> <queries.csv> <k> "
                 ...
                 "   or: fast_vector_validate <vectors.csv> <queries.csv> <k> "
                 "<results.csv> <kernel> hnsw <M> <efConstruction> <efSearch> "
                 "<seed> <simple|heuristic>\n";
    return 2;
  }

  try {
    const auto kernel = argc == 6 ? parse_kernel(argv[5]) : fast_vector::DotProductKernel::Scalar;
    const VectorDataset dataset = load_vectors(argv[1]);
    std::unique_ptr<fast_vector::VectorIndex> index;
    if (argc == 12) {
      if (std::string_view(argv[6]) != "hnsw") {
        throw std::invalid_argument("extended validation mode must be hnsw");
      }
      index = std::make_unique<fast_vector::HnswIndex>(fast_vector::HnswConfig{
          .dimension = dataset.dimension,
          .max_connections = parse_k(argv[7]),
          .ef_construction = parse_k(argv[8]),
          .ef_search = parse_k(argv[9]),
          .random_seed = parse_seed(argv[10]),
          .kernel = kernel,
          .neighbor_selection = parse_neighbor_selection(argv[11]),
      });
    } else {
      index = std::make_unique<fast_vector::FlatIndex>(dataset.dimension, kernel);
    }
    for (std::size_t i = 0; i < dataset.ids.size(); ++i) {
    ...
    for (const auto& [query_id, query] : queries) {
      ...
      for (std::size_t rank = 0; rank < results.size(); ++rank) {
        output << query_id << ',' << rank << ',' << results[rank].id << ',' << results[rank].score
               << '\n';
      }
    }
